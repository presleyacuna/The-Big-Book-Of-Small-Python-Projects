...
DEAD = ' ' # The character representing a deadcell.
...
for x in range(WIDTH): # Loop over every possible column.
    for y in range(HEIGHT): # Loop over every possible row.
        ...
        if random.randint(0,1) == 0: # Add a living cell.
...
while True:  # Main program loop.
    ...
    for y in range(HEIGHT):
        ...
        print() # Print a newlines at the end of the row.
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            if cells[(right, y)] == ALIVE:
                numNeighbors += 1 # Right nighbor is alive.
            ...
            # Set cell based on Conwa's Game of Life rules:
