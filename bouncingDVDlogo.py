...
NOTE: Do not resize the terminal window wihile this program is running.
...
# Set up the constants - get actual terminal size:
...
PAUSE_AMOUNT = .05 # (!) Try changing this to 1.0 or 0.0.
# (!) Try changing this list to fewer colors:
COLORS = ['red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'white'] # a list

UP_RIGHT = 'ur'
UP_LEFT = 'ul'
DOWN_RIGHT = 'dr'
DOWN_LEFT = 'dl'
DIRECTIONS = (UP_RIGHT, UP_LEFT, DOWN_RIGHT, DOWN_LEFT) # a tuple
...
def main():
    ...
    # Generate some logos with initial positions, direction and color.
    logos = []
    for i in range(NUMBER_OF_LOGOS):
        ...
        if logos[-1][X] % 2 == 1: # -1 means the last element in a sequence
            # Make sure X is even so it can hit the corner.
            logos[-1][X] -= 1

    cornerBounces = 0 # Count how many times a logo hits a corner.
    while True: # Main program loop. Calculate position and direction of each of the 5 logos
        for logo in logos: # Handle each logo in the logos list.
            # Erase the logo's current location:
            bext.goto(logo[X], logo[Y])
            print('    ', end='') # (!) try commenting this line out.

            originalDirection = logo[DIR] # gets 1st matching key in the list of dicts = logo[0,"direction"]
        ...
        # Print the 5 logos in their new positions
        for logo in logos:
